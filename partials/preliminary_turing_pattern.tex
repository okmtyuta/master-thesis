...
\subsubsection{Turing pattern and reaction-diffusion equations}
...
In 1952, A. M. Turing mathematically showed, under idealized conditions, that spontaneous spatial patterns can emerge from a homogeneous state through the interaction and diffusion of hypothetical chemical substances known as morphogens \cite{turing1990chemical}. Such phenomena are commonly modeled by the following reaction--diffusion system:
\begin{align}
  &\begin{dcases}\label{eq:general-reaction-diffusion-equations}
    \dfrac{\partial u_i}{\partial t} = \lambda_i \Delta u_i + R_i \left( u_1 , \dots, u_n \right), \\
    1 \leq i \leq n,
  \end{dcases}
\end{align}
where $u_i$ denotes the concentration of the $i$-th morphogen, and $\lambda_i$ is the diffusion coefficient of $u_i$, quantifying the strength of its diffusion. In \eqref{eq:general-reaction-diffusion-equations}, the term $\lambda_i \Delta u_i(x,t)$ describes diffusive transport, while $R_i \left(u_1(x,t),\dots,u_n(x,t)\right)$ represents the chemical reactions. Spatially heterogeneous patterns that arise in reaction--diffusion systems via diffusion-driven instability of a homogeneous steady state are called Turing patterns.

Whether biological body-surface patterns can indeed be explained by Turing patterns had long been a matter of debate. In 1995, however, Kondo and Asai demonstrated that developmental changes in the skin pattern of Pomacanthus fish can be explained by a Turing pattern mechanism. This study supported Turing patterns as an important mathematical model for understanding the emergence of biological body patterns.
...
% \end{align}
% Here, the volume of the reactor is assumed to be constant and equal to $V$. We assume that a reactant $U_1$ with concentration $c$ flows into the reactor at a volumetric flow rate $q$ per unit time. Let $k_1$ and $k_2$ denote the reaction rate constants of reactions \eqref{eq:gs-mutula-reaction} and \eqref{eq:gs-productive-reaction}, respectively, and let $u_1^*$ and $u_2^*$ denote the concentrations of reactants $U_1$ and $U_2$ in the reactor, respectively. Then, including diffusion, the time evolution of $u_1^*$ and $u_2^*$ with respect to time $t^*$ in the reactor is given by
% \begin{align*}
%   \begin{dcases}
%     \dfrac{\partial u_1^*}{\partial t^*} = \lambda_1^* \Delta u_1^* - k_1 {u_1^*}{u_2^*}^2 + \dfrac{q}{V} \left( c - u_1^* \right) ,\\
%     \dfrac{\partial u_2^*}{\partial t^*} = \lambda_2^* \Delta u_2^* + k_1 {u_1^*}{u_2^*}^2 - k_2 u_2^* + \dfrac{q}{V} \left( 0 - u_2^* \right).
%   \end{dcases}
% \end{align*}
...
% F = \dfrac{q / V}{k_1 c^2}, ~
% k = \dfrac{k_2}{k_1 c^2}, ~
% \lambda_1 = \dfrac{\lambda_1^*}{k_1 c^2}, ~
% \lambda_2 = \dfrac{\lambda_2^*}{k_1 c^2},
% \end{align*}
% then we obtain
% \begin{align}
...
\end{align}
Here, assume that the reactor volume is constant and equal to $V$, and that $U_1$ is supplied to the reactor at concentration $c$ with volumetric flow rate $q$. Let $k_1$ and $k_2$ denote the reaction rate constants in \eqref{eq:gs-autocatalytic-reaction} and \eqref{eq:gs-decay-reaction}, respectively, and let $u_1^*$ and $u_2^*$ denote the concentrations of $U_1$ and $U_2$. Including diffusion of the reactants, the evolution of $u_1^*$ and $u_2^*$ is given by
\begin{align}
  \begin{dcases}\label{eq:gs-dimensional}
    \dfrac{\partial u_1^*}{\partial t^*} = \lambda_1^* \Delta u_1^* - k_1 u_1^* (u_2^*)^2 + \dfrac{q}{V}\left(c-u_1^*\right),\\
    \dfrac{\partial u_2^*}{\partial t^*} = \lambda_2^* \Delta u_2^* + k_1 u_1^* (u_2^*)^2 - k_2 u_2^* - \dfrac{q}{V}u_2^* .
  \end{dcases}
\end{align}
Here $\lambda_i^*$ are diffusion coefficients. We nondimensionalize the system by introducing
\begin{align*}
  u_1 = \dfrac{u_1^*}{c},~
  u_2 = \dfrac{u_2^*}{c},~
  t = k_1 c^2 t^*,~
...
\begin{align*}
  F = \dfrac{q/V}{k_1 c^2},~
  k = \dfrac{k_2}{k_1 c^2},~
  \lambda_1 = \dfrac{\lambda_1^*}{k_1 c^2 L^2},~
  \lambda_2 = \dfrac{\lambda_2^*}{k_1 c^2 L^2},
\end{align*}
...
Pearson \cite{pearson1993complex} showed that the Gray--Scott model produces a rich variety of spatiotemporal patterns depending on the parameter choice $(F,k)$. 
Examples of patterns generated by \eqref{eq:general-gray-scott} are shown in Figure~XXX. 
These patterns provide useful insights into diffusion-driven pattern formation in reaction--diffusion systems.


{\color{red}[TODO]Gray--Scottが生成するパターンの画像を用意する}
