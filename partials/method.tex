...
Let $n, d, q$ be positive integers and let $T > 0$ be a positive real number. Set $\Omega = \left[ 0, 1 \right]^d$ and let $\Lambda \subset \left( 0, \infty \right)$ and $\Theta \subset \left( 0, \infty \right)^q$ be compact sets. We consider the following two parameterized reaction-diffusion systems:
\begin{align}
  ...
  \end{dcases}
\end{align}
where $\theta, \theta^\prime \in \Theta$ are parameters of the reaction term. We impose periodic boundary conditions on $\Omega$. We also let $U = \left( u_1, \dots, u_n \right)$ and $U^\prime = \left( u_1^\prime, \dots, u_n^\prime \right)$ be the vector-valued solutions of \eqref{eq:reaction-diffusion-equations-normal} and \eqref{eq:reaction-diffusion-equations-prime}, respectively.

Under this setting, for each $1 \leq i \leq n$, we investigate how the bottleneck distance between the $p$-dimensional persistence diagrams $\mathrm{dgm}_p~u_i \left( \cdot, t \right)$ and $\mathrm{dgm}_p~u_i^\prime \left( \cdot, t \right)$, obtained from $u_i \left( \cdot, t \right)$ and $u_i^\prime \left( \cdot, t \right)$, can be controlled in terms of the model parameters $\lambda_i, \theta$ and $\lambda_i^\prime, \theta^\prime$, where $1 \leq i \leq n$. More precisely, we establish the following Lipschitz continuity.

\begin{thm}\label{thm:main-theorem}
  Let $1 \leq i \leq n$, and let $p \geq 0$ be an integer. Suppose that, there exist constants $L_1, L_2, L_3, L_4 \in \mathbb{R}$ independent of $D, D^\prime$ and $\theta, \theta^\prime$, such that for all $x \in \Omega$ and $0 \leq t \leq T$,
  \begin{align}
    ...
    &\leq L_3, \quad \label{eq:reaction-bound}
    ...
    \leq L_4,\label{eq:state-bound}
  \end{align}
  then, for all $0 \leq t \leq T$, there exist constants $M_1 \left( t \right), M_2 \left( t \right) \in \mathbb{R}$, independent of $D, D^\prime$ and $\theta, \theta^\prime$, such that,
  \begin{align}\label{eq:inequality-main-theorem}
    d_B \left( \mathrm{dgm}_p u_i \left( \cdot, t \right), \mathrm{dgm}_p u_i^\prime \left( \cdot, t \right) \right) \leq M_1 \left( t \right) \left\| D - D^\prime \right\|_\infty + M_2 \left( t \right) \left\| \theta - \theta^\prime \right\|_\infty,
  \end{align}
  where $D = \left( \lambda_1, \dots, \lambda_n \right), D^\prime = \left( \lambda_1^\prime, \dots, \lambda_n^\prime \right) \in \Lambda^n$.
\end{thm}
...
\begin{lem}\label{lem:contraction}
  Let $\lambda \in \Lambda$ and $0 \leq t \leq T$. Then for any $u \in C_{\mathrm{per}} \left( \Omega \right)$, we have
  \begin{align}\label{eq:lem-contraction}
    \left\| H_{\lambda} \left( t \right) u \right\|_\infty \leq  \left\| u \right\|_\infty.
  \end{align}
\end{lem}

\begin{proof}
  This is obvious at $t = 0$. By the definition of $H_{\lambda} \left( t \right)$ , for $0 < t \leq T$, we have
  \begin{align*}
  ...
  \end{align*}
  It follows that \eqref{eq:lem-contraction} holds.
\end{proof}

\begin{lem}\label{lem:heat-kernel-lip}
  For any $0 \leq t \leq T$ and $\lambda, \lambda^\prime \in \Lambda$, we have
  \begin{align}\label{eq:lem-heat-kernel-lip-1}
    \left\| K_{\lambda, t} - K_{\lambda^\prime, t} \right\|_{L^1} \leq \dfrac{d}{\inf_{\lambda^* \in \Lambda} \lambda^*} \left| \lambda - \lambda^\prime \right|.
  \end{align}
  Moreover, for any $u \in C_{\mathrm{per}} \left( \Omega \right)$, we have
  \begin{align}\label{eq:lem-heat-kernel-lip-2}
    \left\| \left( H_{\lambda} \left( t \right) - H_{\lambda^\prime} \left( t \right) \right) u \right\|_{\infty} \leq \dfrac{d}{\inf_{\lambda^* \in \Lambda} \lambda^*} \left| \lambda - \lambda^\prime \right| \left\| u \right\|_\infty.
  \end{align}
\end{lem}

\begin{proof}
  This is obvious at $t = 0$. For any $0 < t \leq T$ and $x \in \mathbb{R}^d$, we can obtainww
  \begin{equation}
    \begin{aligned}\label{eq:lem3-3-1}
    ...
    \end{aligned}
  \end{equation}
  Then, since $\sum_{k \in \mathbb{Z}^d} g_{\lambda, t} \left( x + k \right)$ converges absolutely and uniformly for any $x \in \Omega$ and $\lambda \in \Lambda$, we have
  \begin{align*}
    \dfrac{\partial}{\partial \lambda} K_{\lambda, t}
    &= \dfrac{\partial}{\partial \lambda} \sum_{k \in \mathbb{Z}^d} g_{\lambda, t} \left( x + k \right)\\
    &= \sum_{k \in \mathbb{Z}^d} \dfrac{\partial}{\partial \lambda} g_{\lambda, t} \left( x + k \right).
  \end{align*}
  Taking the $L^1$ norm of both sides, we have
  \begin{align*}
    ...
    &= \sum_{k \in \mathbb{Z}^d} \int_{\Omega} \left| \dfrac{\partial}{\partial \lambda} g_{\lambda, t} \left( x + k \right) \right| dx.
  \end{align*}
  By Tonelli's theorem, we can interchange the sum and the integral, and we obtain
  \begin{align*}
    \left\| \dfrac{\partial}{\partial \lambda} K_{\lambda, t} \right\|_{L^1}
    &= \int_{\Omega} \sum_{k \in \mathbb{Z}^d} \left| \dfrac{\partial}{\partial \lambda} g_{\lambda, t} \left( x + k \right) \right| dx\\
    &= \int_{\mathbb{R}^d} \left| \dfrac{\partial}{\partial \lambda} g_{\lambda, t} \left( x + k \right) \right| dx.
  \end{align*}
  By using \eqref{eq:lem3-3-1}, we obtain
  \begin{align*}
    \left\| \dfrac{\partial}{\partial \lambda} K_{\lambda, t} \right\|_{L^1}
  ...
  \begin{align*}
    ...
    &= \sum_{k \in \mathbb{Z}^d} \int_{\lambda^\prime}^{\lambda} \dfrac{\partial}{\partial \lambda^*} g_{\lambda^*, t} \left( x + k \right) d \lambda^* \\
    &= \int_{\lambda^\prime}^\lambda \sum_{k \in \mathbb{Z}^d} \dfrac{\partial}{\partial \lambda^*} g_{\lambda^*, t} \left( x + k \right) d \lambda^*\\
    &= \int_{\lambda^\prime}^\lambda \dfrac{\partial}{\partial \lambda^*} K_{\lambda^*, t} \left( x \right) d \lambda^*.
  \end{align*}
  Hence, taking the $L^1$ norm of both sides and using Tonelli's theorem, we can obtain
  \begin{align*}
    \left\| K_{\lambda, t} - K_{\lambda^\prime, t} \right\|_{L^1}
    &= \int_{\left[ 0, 1 \right]^d} \left| \int_{\lambda^\prime}^\lambda \dfrac{\partial}{\partial \lambda^*} K_{\lambda^*, t} \left( x \right) d \lambda^* \right| dx\\
    &\leq \int_{\left[ 0, 1 \right]^d} \int_{\lambda^\prime}^\lambda \left| \dfrac{\partial}{\partial \lambda^*} K_{\lambda^*, t} \left( x \right) \right| d \lambda^* dx\\
    &= \int_{\lambda^\prime}^\lambda \left\| \dfrac{\partial}{\partial \lambda^*} K_{\lambda^*, t} \left( x \right) \right\|_{L^1} d\lambda^*\\
    &\leq \sup_{\lambda^* \in \Lambda} \left\| \dfrac{\partial}{\partial \lambda^*} K_{\lambda^*, t} \left( x \right)  \right\|_{L^1} \left| \lambda - \lambda^\prime \right|\\
    &\leq \sup_{\lambda^* \in \Lambda} \dfrac{d}{\lambda^*} \left| \lambda - 
    \lambda^\prime \right|\\
    &= \dfrac{d}{\inf_{\lambda^* \in \Lambda} \lambda^*} \left| \lambda - \lambda^\prime \right|.
  \end{align*}
  Finally, we have
  \begin{align*}
    ...
    &\leq \dfrac{d}{\inf_{\lambda^* \in \Lambda} \lambda^*} \left| \lambda - \lambda^\prime \right| \left\| u \right\|_\infty.
  \end{align*}
  This yields \eqref{eq:lem-heat-kernel-lip-2}
\end{proof}
...
\begin{prop}\label{prop:bound-for-solution-diff}
  For all $0 \leq t \leq T$, there exist constants $M_1 \left( t \right), M_2 \left( t \right) \in \mathbb{R}$, independent of $D, D^\prime$ and $\theta, \theta^\prime$, such that
  \begin{align}\label{eq:prop-bound-for-solution-diff}
...
\begin{proof}
  Since it is obvious at $t = 0$, we consider the case $t > 0$. By the arguments in the Preliminary section, the classical solutions of \eqref{eq:reaction-diffusion-equations-normal} and \eqref{eq:reaction-diffusion-equations-prime} are given by
  \begin{align*}
  ...
  First, by \eqref{eq:state-bound} and Lemma \ref{lem:heat-kernel-lip},
  \begin{align*}
    E_{i, 1}
    &= \left\| \left( H_{\lambda_i} \left( t \right) - H_{\lambda_i^\prime} \left( t \right) \right) \tilde{u}_i \left( \cdot \right) \right\|_\infty \\
    &\leq \dfrac{d}{\inf_{\lambda^* \in \Lambda} \lambda^*} \left| \lambda_i - \lambda_i^\prime \right| \left\| \tilde{u}_i \left( \cdot \right) \right\|_\infty \\
    &\leq \dfrac{d}{\inf_{\lambda^* \in \Lambda} \lambda^*} \left| \lambda_i - \lambda_i^\prime \right| L_4.
  \end{align*}
  ...
  \begin{align*}
    ...
    &\leq \dfrac{d}{\inf_{\lambda^* \in \Lambda} \lambda^*} \left| \lambda_i - \lambda_i^\prime \right| L_3 t.
  \end{align*}
  ...
  \begin{align*}
    ...
    &\leq \dfrac{d}{\inf_{\lambda^* \in \Lambda} \lambda^*} \left( L_4 + L_3 t \right) \left| \lambda_i - \lambda_i^\prime \right| + L_2 t \left\| \theta - \theta^\prime \right\|_\infty + L_1 \int_0^t \left\| U \left( \cdot, s \right) - U^\prime \left( \cdot, s \right) \right\|_\infty ds.
  \end{align*}
  Therefore, we have
  \begin{align*}
    \left\| U \left( \cdot, t \right) - U^\prime \left( \cdot, t \right) \right\|_\infty
    &\leq \dfrac{d}{\inf_{\lambda^* \in \Lambda} \lambda^*} \left( L_4 + L_3 t \right) \left\| D - D^\prime \right\|_\infty + L_2 t \left\| \theta - \theta^\prime \right\|_\infty + L_1 \int_0^t \left\| U \left( \cdot, s \right) - U^\prime \left( \cdot, s \right) \right\|_\infty ds.
  \end{align*}
  By Gronwall's inequality \cite{henry2006geometric}, we obtain
  \begin{align*}
    \left\| U \left( \cdot, t \right) - U^\prime \left( \cdot, t \right) \right\|_\infty
    &\leq \left\{ \dfrac{d}{\inf_{\lambda^* \in \Lambda} \lambda^*} \left( L_4 + L_3 t \right) \left\| D - D^\prime \right\|_\infty + L_2 t \left\| \theta - \theta^\prime \right\|_\infty \right\} e^{L_1 t}.
  \end{align*}
  By setting
  \begin{align*}
  ...
  \begin{align*}
    ...
    \leq M_1 \left( t \right) \left\| D - D^\prime \right\|_\infty + M_2 \left( t \right) \left\| \theta - \theta^\prime \right\|_\infty.
  \end{align*}
  It follows that \eqref{eq:prop-bound-for-solution-diff} holds.
\end{proof}
...
\begin{proof}[Proof of Theorem \ref{thm:main-theorem}]
  ...
  Therefore, 
  \begin{align*}
     d_B \left( \mathrm{dgm}_p~u_i \left( \cdot, t \right), \mathrm{dgm}_p~u_i^\prime \left( \cdot, t \right) \right) \leq M_1 \left( t \right) \left\| D - D^\prime \right\|_\infty + M_2 \left\| \theta - \theta^\prime \right\|_\infty
  \end{align*}
