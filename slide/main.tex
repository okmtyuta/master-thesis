...
\begin{frame}
  ...
  A. M. Turingの研究\cite{turing1990chemical}によれば，このような体表パターンは，モルフォゲンと総称される化学物質の濃度分布が以下のような反応拡散方程式に従って変化することによって生じる。
  \begin{align*}
    \dfrac{ \partial u_i }{ \partial t } = \lambda_i \Delta u_i + R_i \left( u_1, \cdots, u_n; \theta \right) ~ \left( 1 \leq i \leq n \right)
  \end{align*}
  ただし，$u_i$は各モルフォゲンの濃度，$\Delta u_i$は拡散項を表している。また，$R_i$は反応項を表ており，$\theta$は反応項のパラメータである。

  反応拡散系によって生じるパターンは（広い意味で）\alert{チューリングパターン}と呼ばれる。
...
\begin{frame}
  ...
  海水エンゼルフィッシュなどのさまざまな生物のパターンがチューリングパターンによって説明できることが知られている \cite{kondo1995reaction, gravan2004evolving, liu2006two}。生命科学や医学への応用上重要なモデルとなっている。
...
\begin{frame}
  ...
  近年のサーベイ\cite{volkening2024methods}では，チューリングパターン対する定量的解析手法として\alert{トポロジカルデータ解析}が紹介されている。

  トポロジカルデータ解析は，データの「形状」を定量的に調べる手法として近年大きく発展している \cite{carlsson2009topology}。
  
  特に，\alert{パーシステントホモロジー}はトポロジカルデータ解析の代表的手法である \cite{edelsbrunner2002topological}。
...
\begin{frame}
  ...
  パターン形成系に対するパーシステントホモロジーの応用に関する研究はすでに多く行われている\cite{topaz2015topological, nardini2021topological, stolz2022multiscale, mcdonald2023zigzag, mcguirl2020topological, yang2025topological, kanamori2025analysis, spector2024persistent}。

  特に，\cite{spector2024persistent}ではパーシステントホモロジーを活用して，反応拡散系のパラメータをクラスタリングしている。
...
\begin{frame}
  ...
  本研究の目的は，\alert{\textbf{反応拡散系のパラメータの類似性とパーシステントホモロジーの類似性の間の関係性を数理的に解析すること}}である。
...
\begin{frame}
  ...
  \begin{thm*}\label{thm:main-theorem}
    $D, D^\prime$および$\theta, \theta^\prime$に依存しない定数$L_1, L_2, L_3, L_4 \in \mathbb{R}$が存在して，任意の$x \in \Omega$および$0 \leq t \leq T$に対して，
    \begin{align*}
...
\begin{frame}
  ...
  \end{align*}
  が成り立つことが知られている（安定性定理）。
  
  よって，ボトルネック距離を評価するためには，$\left\| f - g \right\|_\infty$を評価すればいい。
...
\begin{frame}
  ...
  代表的な反応拡散系であるGray--Scottモデルに対して，主定理に関する数値シミュレーションを行った。Gray--Scottモデルは次で与えられる。
  \begin{align*}
    \begin{dcases}
...
\begin{frame}
  ...
  \begin{minipage}[t]{0.35\linewidth}
    ...
    \begin{itemize}
      ...
      \item パーシステンス図の類似性
    \end{itemize}
    パラメータが類似していれば，パーシステンス図の店の配置も類似していることがわかる。
  \end{minipage}
  \begin{minipage}[t]{0.60\linewidth}
    \vspace{0pt}
    \includegraphics[width=\linewidth]{figures-thesis/view_u_tilde_series.png}
  \end{minipage}

\end{frame}

\begin{frame}
  Gray--Scottモデルに対して，以下の不等式をシミュレーションした。
  \begin{align*}
    ...
    &= M_2 \left( t \right) \left\| \theta - \theta^\prime \right\|_\infty
  \end{align*}
  その結果を以下に示す。緑が不等式の右辺であり，オレンジが不等式の左辺を意味している。ただし，結果は対数スケールで示している。不等式は一応成立している。
  \begin{figure}[h]
    \begin{minipage}{0.32\linewidth}
      \centering
...
\begin{frame}
  ...
  \alert{\textbf{反応拡散系のパラメータの類似性とパーシステントホモロジーの類似性の間の関係性を数理的に解析する}}という研究目的は，パーシステンス図とパラメータの間のリプシッツ連続性（以下の不等式）として一応達成された。
  \begin{align*}
    d_B \left( \mathrm{dgm} u_i \left( \cdot, t \right), \mathrm{dgm} u_i^\prime \left( \cdot, t \right) \right) \leq M_1 \left( t \right) \left\| D - D^\prime \right\|_\infty + M_2 \left( t \right) \left\| \theta - \theta^\prime \right\|_\infty,
  \end{align*}
...
\begin{frame}
  ...
  本研究における反応拡散系は，有限時間区間$\left[ 0, T \right]$で考えていたが，実際の反応拡散系の研究では，十分に時間がたった定常状態を考えることも少なくない。

  しかし，時間$t$が大きくなると$M_1 \left( t \right)$および$M_2 \left( t \right)$が指数的に大きくなってしまうため，このような状況を考えることができない。
  \begin{align*}
    d_B \left( \mathrm{dgm} u_i \left( \cdot, t \right), \mathrm{dgm} u_i^\prime \left( \cdot, t \right) \right) \leq \alert{M_1 \left( t \right)} \left\| D - D^\prime \right\|_\infty + \alert{M_2 \left( t \right)} \left\| \theta - \theta^\prime \right\|_\infty
  \end{align*}
...
\begin{frame}
  ...
  \begin{align*}
    ...
    &\leq M_1 \left( t \right) \left\| D - D^\prime \right\|_\infty + M_2 \left( t \right) \left\| \theta - \theta^\prime \right\|_\infty
  \end{align*}
  これではただ解の性質を調べているだけで，データの「形」を調べるというパーシステントホモロジーの性質を本質的には利用できていない。
